...
public class Calculator {
    ...
    public void run() {
        ...
        screenWritter = new ScreenWritter();
        screenWritter.showIntro();
        String option = "";
        while (!option.equals("x")) {
            screenWritter.showInstructions();
            option = scanner.nextLine();
            switch (option) {
                case "+":
                    performCallculation(option);
                    break;
                case "-":
                    performCallculation(option);
                    break;
    ...
    private void performCallculation(String option) {
        screenWritter.showOperation(option);
        number1 = readNumberFromUser("number1");
        number2 = readNumberFromUser("number2");
        result = getResult(number1, number2, option);
        screenWritter.showResult(number1, number2, option, result);
    ...
    /**
     * Načíta konkrétne číslo od používaľa
     */
    private float readNumberFromUser(String numberName) {
        float number;
        while (true) {
            System.out.println("Enter " + numberName);
